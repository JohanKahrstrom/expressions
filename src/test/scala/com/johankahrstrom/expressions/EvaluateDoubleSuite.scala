...
class EvaluateDoubleSuite extends FunSuite {
  test("Zero is 0.0") {
    assert(Zero[Double].evaluate(10) === 0.0)
  }

  test("One is 1.0") {
    assert(One[Double].evaluate(20.0) === 1.0)
    assert(One[Int].evaluate(30) === 1.0)
  }
